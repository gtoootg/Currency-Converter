...
function InputField({ currencyValue, setCurrencyValue }) {
  return (
    <input
      ...
      value={
        currencyValue
          ? Intl.NumberFormat('de-DE', { style: 'decimal' }).format(currencyValue)
          : null
      }
...
export default function Converter(prop) {
  ...
  function fetchCurrencyList() {
    ...
    axios
      .get('http://api.exchangeratesapi.io/v1/latest?access_key=3298054fd6cea8d6cbc4f26298ad9f2d')
      .then((res) => {
  ...
  function DisplayCurrencyList({ initialCurrency, currentCurrency, setCurrency }) {
    return (
      <select
        ...
        data-testid="select"
      >
        <option value={initialCurrency} selected>
          {currencyList[initialCurrency]}
        </option>
        {Object.keys(currencyList).map((list, i) => (
          <option key={i} value={list} data-testid={list}>
            {currencyList[list]}
          </option>
        ))}
      </select>
  ...
  return (
    <>
      <div>
        <InputField currencyValue={upperCurrencyValue} setCurrencyValue={setUpperCurrencyValue} />
        <DisplayCurrencyList
          initialCurrency="EUR"
      ...
      <br />
      <div>
        <InputField currencyValue={lowerCurrencyValue} setCurrencyValue={setLowerCurrencyValue} />
        <DisplayCurrencyList
          initialCurrency="USD"
